    ...
    async getUser(username, id) {
        if (this.preferV2 && this.v2Available && this.osuApiV2) {
            try {
                ...
                return await OsuApiV1.getUser(username, id);
            }
        } else {
            return await OsuApiV1.getUser(username, id);
        }
    ...
    async getTop100MultiMods(userId, id) {
        if (this.preferV2 && this.v2Available && this.osuApiV2) {
            try {
                ...
                return await this.osuApiV2.getTop100MultiMods(userId, id);
            } catch (error) {
                Logger.errorCatch('OsuApiWrapper', `V2 getTop100MultiMods failed, falling back to V1: ${error.message}`);
                return await OsuApiV1.getTop100MultiMods(userId, id);
            }
        } else {
    ...
    async getBeatmap(beatmapId) {
        if (this.preferV2 && this.v2Available && this.osuApiV2) {
            try {
                ...
                return await OsuApiV1.getBeatmap(beatmapId);
            }
        } else {
            return await OsuApiV1.getBeatmap(beatmapId);
        }
    ...
    async hasUserPlayedMap(userId, beatmapId) {
        if (this.preferV2 && this.v2Available && this.osuApiV2) {
            try {
                ...
                return await OsuApiV1.hasUserPlayedMap(userId, beatmapId);
            }
        } else {
            return await OsuApiV1.hasUserPlayedMap(userId, beatmapId);
        }
